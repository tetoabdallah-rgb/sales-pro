...
if sbb_old not in html:
    print('Error: sbb_old not found')
    sys.exit(1)
html = html.replace(sbb_old, sbb_new)

# 2. Update dateFilterUI
df_old = '''    let dateFilterUI = `
...
df_new = '''    let dateFilterUI = `
    ...
    `;'''
# note: in the old version, the clear button text might be different. Let's just find the block by a simpler regex.
import re
df_pattern = r'let dateFilterUI = `[\s\S]*?</div>\s*`;'
if not re.search(df_pattern, html):
    print('Error: df_pattern not found')
    sys.exit(1)
html = re.sub(df_pattern, df_new.strip(), html)

# 3. Add setDates logic
sd_old = '''    $('bDateClear').onclick = () => {
        globalDateRange = { start: null, end: null };
        rDash();
    };'''
sd_new = '''    $('bDateClear').onclick = () => {
        globalDateRange = { start: null, end: null };
        rDash();
    };
...
if sd_old not in html:
    print('Error: sd_old not found')
    sys.exit(1)
html = html.replace(sd_old, sd_new)

# 4. Add updateAlertBadge and call in render
rb_old = '''function render() {
    let fn = {
...
rb_new = '''function updateAlertBadge() {
    if(!$('alBadge')) return;
    let alertsCount = 0;
    let today = new Date();
    let cu = {};
...
if rb_old not in html:
    print('Error: rb_old not found')
    sys.exit(1)
html = html.replace(rb_old, rb_new)

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print('Successfully restored and injected code.')
