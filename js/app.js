...
function buildNav() {
    ...
    if (currentRole === 'admin') {
        ...
        // Hide companies and users management from sub-admins bound to a specific company
        if (!userData.company || userData.company === 'Main') {
...
async function loadAllSales() {
    ...
    try {
        ...
        snap.docs.forEach(doc => {
            let d = doc.data();
            if (userData.company && userData.company !== 'Main' && d.company !== userData.company) return;
...
async function loadAdminUsers(roleFilter) {
    ...
    db.collection('auth_users').onSnapshot(snap1 => {
        ...
        db.collection('users').where('adminId', '==', userData.adminId || currentUser.uid).onSnapshot(snap2 => {
            ...
            cachedAdminUsers.forEach(u => {
                ...
                if (userData.company && userData.company !== 'Main' && u.company !== userData.company) return;
...
async function loadAdminProducts() {
    ...
    tb.innerHTML = `<tr><th>${t('company')}</th><th>${t('sale_code')}</th><th>${t('sale_desc')}</th><th>${t('sale_price')}</th><th>${t('delete')}</th></tr>`;
    $('loader').classList.remove('hidden');
    db.collection('products').where('adminId', '==', userData.adminId || currentUser.uid).limit(100).onSnapshot(snap => {
        let htmlAcc = "";
        snap.docs.forEach(doc => {
            let p = doc.data();
            if (userData.company && userData.company !== 'Main' && p.company !== userData.company) return;
...
function loadAdminStock() {
    ...
    $('loader').classList.remove('hidden');
    db.collection('company_stock').where('adminId', '==', userData.adminId || currentUser.uid).limit(100).onSnapshot(snap => {
        let htmlAcc = "";
        snap.docs.forEach(doc => {
            let p = doc.data();
            if (userData.company && userData.company !== 'Main' && p.company !== userData.company) return;
...
async function loadAllAttendance() {
    ...
    try {
        ...
        snap.docs.forEach(doc => {
            ...
            if (userData.company && userData.company !== 'Main' && (!user || user.company !== userData.company)) return;
...
function loadCompaniesManagerUI() {
    db.collection('companies_list').where('adminId', '==', currentUser.uid).onSnapshot(snap => {
        let cont = $('cmpList');
        if (!cont) return;
        cont.innerHTML = '';
        snap.forEach(doc => {
...
function showCompanyDetails(id, name) {
    ...
    // Load Promoters
    db.collection('users').where('company', '==', name).where('adminId', '==', currentUser.uid).onSnapshot(snap => {
