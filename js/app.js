...
function nav(page) {
    ...
    if (page === 'promoter_dash') {
        ...
        title = t('admin_sales');
        content = `<div class="glass-card" style="margin-bottom: 20px;"><div class="grid-3"><div class="fg"><label>من تاريخ</label><input type="date" id="salesStartDate" class="input-box"></div><div class="fg"><label>إلى تاريخ</label><input type="date" id="salesEndDate" class="input-box"></div><div class="fg" style="display:flex; align-items:flex-end;"><button class="btn btn-primary" onclick="loadAllSales()">تطبيق الفلتر 🔍</button></div></div></div><div class="search-bar"><input type="text" id="s_allSales" class="input-box" placeholder="${t('search')}..." onkeyup="filterTable('s_allSales', 'allSalesTable')"><button class="btn export-btn" onclick="exportTableToExcel('allSalesTable', 'Sales')">${t('export_excel')}</button></div><div class="glass-card" style="overflow-x:auto;"><table id="allSalesTable"></table></div>`;
...
async function loadAdminProducts() {
    ...
    $('loader').classList.remove('hidden');
    db.collection('products').where('adminId', '==', currentUser.uid).limit(100).onSnapshot(snap => {
...
function loadAdminStock() {
    ...
    $('loader').classList.remove('hidden');
    db.collection('company_stock').where('adminId', '==', currentUser.uid).limit(100).onSnapshot(snap => {
