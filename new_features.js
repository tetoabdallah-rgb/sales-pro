...
window.addVisitModal = function() {
    ...
    let set2 = T_data.map(r => r.Customer || r['Customer Name']).filter(Boolean);
...
window.saveLead = function() {
    ...
    if(!n) { alert(L==='ar'?'الرجاء إدخال اسم العميل':'Please enter lead name'); return; }
    ...
    if(modal) modal.remove();
    loadLeads();
    if(typeof toast === 'function') toast(L==='ar'?'تم إضافة العميل بنجاح':'Lead added successfully', 'success');
...
window.handleTargetImport = function(e) {
    ...
    reader.onload = ev => {
        try {
            ...
            alert(L==='ar'?'تم استيراد البيانات بنجاح!':'Data imported successfully!');
            if(typeof window.rTgt === 'function') window.rTgt();
            if(typeof window.cloudAutoSave === 'function') window.cloudAutoSave(L==='ar'?'استيراد تارجت من إكسيل':'Target imported from Excel');
        } catch(err) {
...
window.rTgt = function() {
    ...
    if(tData && tData.length > 0) {
        tData.forEach(r => { 
            let cName = r.Customer || r['Customer Name'] || '';
            tt += Number(r.Target)||0; 
            ta += cS(cName); 
            hwt += Number(r.hwTarget)||0;
            acct += Number(r.accTarget)||0;
            hwa += cSF(cName, window.isHW);
            acca += cSF(cName, window.isAcc);
        });
    }
    ...
    mDiv.innerHTML = `
        ...
        <div class="tb">
            <div class="tbt"><h3>${typeof t==='function'?t('targets'):(L==='ar'?'المستهدفات':'Targets')}</h3><input class="sbox" id="tsr" placeholder="${L==='ar'?'بحث في المستهدفات...':'Search targets...'}"></div>
            <div class="tbs">
                <table style="width:100%;text-align:left;border-collapse:collapse;white-space:nowrap;">
                    <thead>
                        <tr>
    ...
    if(document.getElementById('bExTgt') && typeof exportToExcel === 'function') {
        document.getElementById('bExTgt').onclick = () => exportToExcel(tData.map(r => ({ Customer: r.Customer || r['Customer Name'], Phone: r.phone||r.Phone||'', Address: r.address||r.Address||'', Target: Number(r.Target)||0, Achieved: cS(r.Customer || r['Customer Name']), HW_Target: r.hwTarget||0, HW_Achieved: cSF(r.Customer || r['Customer Name'],window.isHW), Acc_Target: r.accTarget||0, Acc_Achieved: cSF(r.Customer || r['Customer Name'],window.isAcc) })), 'Targets_Report');
    }
    ...
    function fTg(d){
        ...
        ttb.innerHTML = d.map(r => {
            let cName = r.Customer || r['Customer Name'] || '';
            let tg = Number(r.Target)||0, a = cS(cName), p = tg>0 ? a/tg*100 : 0;
            let hwT = Number(r.hwTarget)||0, hwA = cSF(cName,window.isHW);
            let accT = Number(r.accTarget)||0, accA = cSF(cName,window.isAcc);
            ...
            let address = r.address || r.Address || '';
            let contactHTML = '';
            if(phone || address) {
                contactHTML = `<div style="font-size:0.8rem; color:var(--tx2); margin-top:4px;">📞 ${phone||'-'} <br/>📍 ${address||'-'}</div>`;
            }
            
            let encCust = encodeURIComponent(cName);
            return `<tr>
                <td style="font-weight:bold;">${cName}${contactHTML}</td>
                ...
                <td>
                    <button onclick="window.openCustomerProfile(decodeURIComponent('${encCust}'))" class="btn btn-p" style="padding:4px 8px; font-size:0.8rem; margin-right:5px; font-family:inherit; cursor:pointer;">
                        👤 ${L==='ar'?'بروفايل':'Profile'}
                    </button>
                    <button onclick="window.editCustomerTarget(decodeURIComponent('${encCust}'))" class="btn" style="padding:4px 8px; font-size:0.8rem; background:var(--bg3); border:1px solid var(--bd); font-family:inherit; cursor:pointer;">
                        ${L==='ar'?'تعديل':'Edit'}
                    </button>
                </td>
            </tr>`;
...
(function initGlobalFonts() {
    ...
    window.setAppFontSize = function(idx) {
        ...
        window.applyAppFontSettings();
        if (typeof toast === 'function') toast(L==='ar'?'تم تغيير حجم الخط':'Font size changed');
    };
    ...
    window.setAppFontFamily = function(fontName) {
        ...
        window.applyAppFontSettings();
        if (typeof toast === 'function') toast(L==='ar'?'تم تغيير نوع الخط':'Font family changed');
    };
})();
...
window.editCustomerTarget = function(cName) {
    try {
        if (typeof cName === 'string' && (cName.startsWith('%') || cName.includes('%'))) {
            try { cName = decodeURIComponent(cName); } catch(e) {}
        }
    } catch(e) {}
    ...
    let norm = (s) => (s || '').toString().trim().toLowerCase();
    let targetNorm = norm(cName);
    
    let customerData = (cName ? tData.find(r => norm(r.Customer) === targetNorm || norm(r['Customer Name']) === targetNorm || norm(r.Name) === targetNorm) : null) || { Customer: '', phone: '', address: '', hwTarget: 0, accTarget: 0, Target: 0 };
    ...
    modal.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.6);z-index:100000;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px);padding:15px;';
    
    modal.onclick = (e) => {
        if (e.target === modal) modal.remove();
    };
    
    let title = cName ? (L==='ar'?'تعديل بيانات العميل والمستهدف':'Edit Customer & Targets') : (L==='ar'?'إضافة عميل ومستهدف جديد':'Add New Customer & Target');
    let safeVal = (v) => (v == null ? '' : String(v).replace(/"/g, '&quot;'));
    let initialTotal = (Number(customerData.hwTarget||0) + Number(customerData.accTarget||0)) || Number(customerData.Target||0);
    
    modal.innerHTML = `
        <div class="card" style="width:100%; max-width:520px; padding:25px; border-radius:18px; background:var(--bg); box-shadow:0 20px 60px rgba(0,0,0,0.5); position:relative; max-height:90vh; overflow-y:auto; border:1px solid var(--bd);">
            <button onclick="document.getElementById('sp_tgt_modal').remove()" style="position:absolute;top:15px;right:15px;background:none;border:none;font-size:1.6rem;color:var(--tx2);cursor:pointer;padding:4px 8px;line-height:1;">&times;</button>
            <h2 style="margin-bottom:20px; display:flex; align-items:center; gap:10px; font-size:1.3rem;"><span style="font-size:1.4rem;">🎯</span> ${title}</h2>
            
            <label class="sp-form-label" style="display:block;margin-bottom:6px;font-weight:bold;color:var(--tx1);font-size:0.9rem;">${L==='ar'?'اسم العميل':'Customer Name'} <span style="color:#ef4444">*</span></label>
            <input type="text" id="tgt_name" class="sbox" value="${safeVal(customerData.Customer || customerData['Customer Name'] || cName)}" ${cName?'disabled':''} style="width:100%;padding:11px;margin-bottom:14px;border-radius:8px;border:1px solid var(--bd);background:${cName?'var(--bg2)':'var(--bg)'};color:var(--tx1);font-size:0.95rem;">
            
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; margin-bottom:14px;">
                <div>
                    <label class="sp-form-label" style="display:block;margin-bottom:6px;font-weight:bold;color:var(--tx1);font-size:0.85rem;">${L==='ar'?'رقم الهاتف':'Phone'}</label>
                    <input type="tel" id="tgt_phone" class="sbox" value="${safeVal(customerData.phone || customerData.Phone || '')}" style="width:100%;padding:11px;border-radius:8px;border:1px solid var(--bd);background:var(--bg);color:var(--tx1);font-size:0.95rem;">
                </div>
                <div>
                    <label class="sp-form-label" style="display:block;margin-bottom:6px;font-weight:bold;color:var(--tx1);font-size:0.85rem;">${L==='ar'?'العنوان / المنطقة':'Address / Region'}</label>
                    <input type="text" id="tgt_address" class="sbox" value="${safeVal(customerData.address || customerData.Address || '')}" style="width:100%;padding:11px;border-radius:8px;border:1px solid var(--bd);background:var(--bg);color:var(--tx1);font-size:0.95rem;">
                </div>
            </div>
            
            <div style="background:var(--bg2); padding:16px; border-radius:12px; margin-bottom:20px; border:1px solid var(--bd);">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                    <h4 style="margin:0; color:var(--tx); font-size:0.95rem;">${L==='ar'?'المستهدفات (Targets)':'Target Values'}</h4>
                    <span id="tgt_total_preview" style="font-weight:bold; color:var(--ac); font-size:0.9rem;">${L==='ar'?'الإجمالي: ':'Total: '}${initialTotal.toLocaleString()}</span>
                </div>
                
                <label class="sp-form-label" style="display:block;margin-bottom:6px;font-weight:bold;color:#ff9800;font-size:0.85rem;">💻 ${L==='ar'?'تارجت الهاردوير (HW Target)':'Hardware Target (HW)'}</label>
                <input type="number" id="tgt_hw" class="sbox" value="${Number(customerData.hwTarget)||0}" oninput="window.updateTgtPreview()" style="width:100%;padding:11px;margin-bottom:12px;border-radius:8px;border:1px solid var(--bd);background:var(--bg);color:var(--tx1);font-size:0.95rem;">
                
                <label class="sp-form-label" style="display:block;margin-bottom:6px;font-weight:bold;color:#10b981;font-size:0.85rem;">🎧 ${L==='ar'?'تارجت الإكسسوارات (Acc Target)':'Accessories Target (Acc)'}</label>
                <input type="number" id="tgt_acc" class="sbox" value="${Number(customerData.accTarget)||0}" oninput="window.updateTgtPreview()" style="width:100%;padding:11px;border-radius:8px;border:1px solid var(--bd);background:var(--bg);color:var(--tx1);font-size:0.95rem;">
            </div>
            
            <button onclick="window.saveCustomerTarget(decodeURIComponent('${encodeURIComponent(cName || '')}'))" class="btn btn-p" style="width:100%; padding:13px; font-size:1rem; font-weight:bold; border-radius:10px; display:flex; justify-content:center; align-items:center; gap:8px; cursor:pointer;">
                <span>💾</span> ${L==='ar'?'حفظ التعديلات والتحديث':'Save Target Changes'}
            </button>
        </div>
    `;
    
    window.updateTgtPreview = function() {
        let hw = Number(document.getElementById('tgt_hw')?.value) || 0;
        let acc = Number(document.getElementById('tgt_acc')?.value) || 0;
        let el = document.getElementById('tgt_total_preview');
        let L_curr = (typeof localStorage !== 'undefined' && localStorage.getItem('sp_lang')) || 'ar';
        if (el) el.textContent = (L_curr==='ar'?'الإجمالي: ':'Total: ') + (hw + acc).toLocaleString();
    };
...
window.openCustomerProfile = function(cName) {
    ...
    let L = (typeof localStorage !== 'undefined' && localStorage.getItem('sp_lang')) || 'ar';
    let sData = typeof getS === 'function' ? getS() : (window.S || []);
    let cData = typeof getC === 'function' ? getC() : (typeof loadLS === 'function' ? loadLS('payData') : (window.C || []));
    let tData = typeof getT === 'function' ? getT() : (window.T || []);
    ...
    // 1. Get Master Data
    let tRow = tData.find(r => norm(r.Customer) === targetNorm || norm(r['Customer Name']) === targetNorm || norm(r.Name) === targetNorm || norm(r.Merchant) === targetNorm) || { Customer: cName, phone: '', address: '' };
    
    let hwTarget = Number(tRow.hwTarget) || 0;
    let accTarget = Number(tRow.accTarget) || 0;
    let targetValue = Number(tRow.Target || tRow['Target Value'] || tRow.target || 0);
    if ((hwTarget > 0 || accTarget > 0) && (!targetValue || targetValue === 0)) {
        targetValue = hwTarget + accTarget;
    }
    
    // 2. Aggregate Sales
    let cSales = sData.filter(r => norm(r.Customer || r['Customer Name']) === targetNorm);
    let totalSales = 0;
    let hwSales = 0;
    let accSales = 0;
    
    cSales.forEach(s => {
        let val = typeof getSalesVal === 'function' ? getSalesVal(s) : (Number(s['Sales After Discount'] || s['Sales Without Tax'] || s['Sales'] || s['Amount'] || s.Value) || 0);
        totalSales += val;
        let cls = s['Item Class Name'] || s.Category || '';
        let isH = typeof isHW === 'function' ? isHW(cls) : cls.toLowerCase().includes('hw');
        let isA = typeof isAcc === 'function' ? isAcc(cls) : cls.toLowerCase().includes('acc');
        if (!isH && !isA) {
            if (val > 1000) isH = true; else isA = true;
        }
        if (isH) hwSales += val;
        if (isA) accSales += val;
    });
    ...
    let cColls = cData.filter(r => {
        let keys = Object.keys(r);
        let k = keys.find(k => ['customer name', 'customer', 'name', 'اسم العميل', 'العميل'].includes(k.toLowerCase().replace(/\s+/g, '')));
        let cn = k ? r[k] : (r.Customer || r['Customer Name'] || '');
        return norm(cn) === targetNorm;
    });
    
    let totalColls = 0;
    cColls.forEach(c => {
        let val = typeof getPayVal === 'function' ? getPayVal(c) : 0;
        if (!val) {
            let keys = Object.keys(c);
            let k = keys.find(k => ['amount', 'collection', 'value', 'المبلغ', 'التحصيل'].includes(k.toLowerCase().replace(/\s+/g, '')));
            if (k) {
                let v = c[k];
                val = typeof v === 'string' ? Number(v.replace(/,/g, '')) || 0 : Number(v) || 0;
            }
        }
        totalColls += val;
    });
    ...
    let existing = document.getElementById('sp_customer_profile_modal');
    if (existing) existing.remove();
    
    let modal = document.createElement("div");
    modal.id = 'sp_customer_profile_modal';
    modal.className = "sp-modal-overlay";
    modal.style.cssText = "position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);backdrop-filter:blur(5px);z-index:100000;display:flex;align-items:center;justify-content:center;overflow-y:auto;padding:20px;";
    
    modal.onclick = (e) => {
        if (e.target === modal) modal.remove();
    };
    
    let safeFmt = (n) => typeof fmt === 'function' ? fmt(n) : Number(n||0).toLocaleString('en-US', {maximumFractionDigits: 0});
    let safePd = (v) => typeof pd === 'function' ? pd(v) : (v ? String(v) : '--');
    
    let sortedSales = cSales.slice().sort((a,b) => {
        let da = new Date(safePd(a['Invoice Date'] || a['Order Date'] || a['Date'])).getTime() || 0;
        let db = new Date(safePd(b['Invoice Date'] || b['Order Date'] || b['Date'])).getTime() || 0;
        return db - da;
    });
    
    let sortedColls = cColls.slice().sort((a,b) => {
        let da = new Date(safePd(a['Date'] || a['Payment Date'] || a['Invoice Date'])).getTime() || 0;
        let db = new Date(safePd(b['Date'] || b['Payment Date'] || b['Invoice Date'])).getTime() || 0;
        return db - da;
    });
    
    let salesRows = sortedSales.length > 0 ? sortedSales.map(s => {
        let val = typeof getSalesVal === 'function' ? getSalesVal(s) : (Number(s['Sales After Discount'] || s['Sales Without Tax'] || s['Sales'] || s['Amount'] || s.Value) || 0);
        let itm = s['Item Description'] || s['Item Class Name'] || s['Item Name'] || s.item || '--';
        let dt = safePd(s['Invoice Date'] || s['Order Date'] || s['Date']) || '--';
        return `<tr>
            <td style="padding:10px; border-bottom:1px solid var(--bd); font-size:0.85rem; color:var(--tx2);">${dt}</td>
            <td style="padding:10px; border-bottom:1px solid var(--bd); font-size:0.85rem; max-width:200px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="${itm}">${itm}</td>
            <td style="padding:10px; border-bottom:1px solid var(--bd); font-weight:bold; color:var(--p); font-size:0.9rem;">${safeFmt(val)}</td>
        </tr>`;
    }).join('') : `<tr><td colspan="3" style="padding:20px; text-align:center; color:var(--tx2);">${L==='ar'?'لا توجد مبيعات مسجلة':'No sales records'}</td></tr>`;
    
    let collRows = sortedColls.length > 0 ? sortedColls.map(c => {
        let val = typeof getPayVal === 'function' ? getPayVal(c) : (Number(c.Amount || c.Collection || c.Value) || 0);
        let note = c['Payment Ref.'] || c['Payment Ref'] || c.Notes || c.notes || c['PaymentRef'] || '--';
        let dt = safePd(c['Date'] || c['Payment Date'] || c['Invoice Date']) || '--';
        return `<tr>
            <td style="padding:10px; border-bottom:1px solid var(--bd); font-size:0.85rem; color:var(--tx2);">${dt}</td>
            <td style="padding:10px; border-bottom:1px solid var(--bd); font-size:0.85rem; color:var(--tx2);">${note}</td>
            <td style="padding:10px; border-bottom:1px solid var(--bd); font-weight:bold; color:var(--ac); font-size:0.9rem;">${safeFmt(val)}</td>
        </tr>`;
    }).join('') : `<tr><td colspan="3" style="padding:20px; text-align:center; color:var(--tx2);">${L==='ar'?'لا توجد تحصيلات مسجلة':'No collections records'}</td></tr>`;
    
    let achPct = targetValue > 0 ? ((totalSales / targetValue) * 100).toFixed(1) : 0;
    
    let content = `
    <div class="sp-modal-content" style="background:var(--bg); color:var(--tx); border-radius:20px; padding:25px; width:100%; max-width:920px; box-shadow:0 20px 60px rgba(0,0,0,0.5); border:1px solid var(--bd); max-height:90vh; overflow-y:auto; position:relative;">
        <!-- Header -->
        <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:2px solid var(--bd); padding-bottom:15px; margin-bottom:20px;">
            <div style="display:flex; align-items:center; gap:15px;">
                <div style="width:52px; height:52px; border-radius:50%; background:linear-gradient(135deg, var(--p), var(--ac)); display:flex; align-items:center; justify-content:center; color:#fff; font-size:1.6rem; font-weight:bold; flex-shrink:0;">
                    ${(cName || '👤').charAt(0)}
                </div>
                <div>
                    <h2 style="margin:0; font-size:1.4rem; font-weight:800; color:var(--tx);">${cName}</h2>
                    <p style="margin:4px 0 0; color:var(--tx2); font-size:0.85rem;">
            ...
            <div style="display:flex; align-items:center; gap:10px;">
                <button onclick="window.editCustomerTarget(decodeURIComponent('${encodeURIComponent(cName)}'))" class="btn" style="padding:6px 12px; font-size:0.85rem; background:var(--bg3); border:1px solid var(--bd); border-radius:8px; cursor:pointer; color:var(--tx1);">
                    ✏️ ${L==='ar'?'تعديل التارجت':'Edit Target'}
                </button>
                <button onclick="document.getElementById('sp_customer_profile_modal').remove()" style="background:transparent; border:none; color:var(--tx2); font-size:1.8rem; cursor:pointer; line-height:1; padding:4px 8px;">&times;</button>
            </div>
        </div>
        
        <!-- KPIs -->
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(180px, 1fr)); gap:12px; margin-bottom:24px;">
            <div style="background:var(--bg2); padding:14px; border-radius:12px; border:1px solid var(--bd); text-align:center;">
                <div style="margin:0 0 4px; color:var(--tx2); font-size:0.8rem; font-weight:bold;">${L==='ar'?'الهدف (التارجت)':'Target'}</div>
                <div style="font-size:1.4rem; font-weight:bold; color:var(--am);">${safeFmt(targetValue)}</div>
                <div style="font-size:0.75rem; color:var(--tx2); margin-top:2px;">💻 ${safeFmt(hwTarget)} | 🎧 ${safeFmt(accTarget)}</div>
            </div>
            <div style="background:var(--bg2); padding:14px; border-radius:12px; border:1px solid var(--bd); text-align:center;">
                <div style="margin:0 0 4px; color:var(--tx2); font-size:0.8rem; font-weight:bold;">${L==='ar'?'المسحوبات (المبيعات)':'Total Sales'}</div>
                <div style="font-size:1.4rem; font-weight:bold; color:var(--p);">${safeFmt(totalSales)}</div>
                <div style="font-size:0.75rem; color:var(--gn); font-weight:bold; margin-top:2px;">
                    ${targetValue > 0 ? achPct + (L==='ar'?'% تحقيق':'% Achieved') : '-'}
                </div>
            </div>
            <div style="background:var(--bg2); padding:14px; border-radius:12px; border:1px solid var(--bd); text-align:center;">
                <div style="margin:0 0 4px; color:var(--tx2); font-size:0.8rem; font-weight:bold;">${L==='ar'?'إجمالي التحصيلات':'Collections'}</div>
                <div style="font-size:1.4rem; font-weight:bold; color:var(--ac);">${safeFmt(totalColls)}</div>
                <div style="font-size:0.75rem; color:var(--tx2); margin-top:2px;">${cColls.length} ${L==='ar'?'دفعات':'payments'}</div>
            </div>
            <div style="background:var(--bg2); padding:14px; border-radius:12px; border:1px solid var(--bd); text-align:center;">
                <div style="margin:0 0 4px; color:var(--tx2); font-size:0.8rem; font-weight:bold;">${L==='ar'?'الرصيد المتبقي (المديونية)':'Balance (Due)'}</div>
                <div style="font-size:1.4rem; font-weight:bold; color:${balance > 0 ? '#ef4444' : '#10b981'};">${safeFmt(balance)}</div>
                <div style="font-size:0.75rem; color:${balance > 0 ? '#ef4444' : '#10b981'}; font-weight:bold; margin-top:2px;">
                    ${balance > 0 ? (L==='ar'?'مستحق سداد':'Pending Due') : (L==='ar'?'مسدد بالكامل':'Settled')}
                </div>
            </div>
        </div>

        <!-- History Tables -->
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(320px, 1fr)); gap:16px; margin-bottom:15px;">
            <!-- Sales -->
            <div style="background:var(--bg2); border-radius:14px; border:1px solid var(--bd); padding:16px; overflow:hidden;">
                <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid var(--bd); padding-bottom:10px; margin-bottom:12px;">
                    <h3 style="margin:0; font-size:1rem; color:var(--tx);">🛒 ${L==='ar'?'سجل الفواتير':'Sales History'} (${cSales.length})</h3>
                </div>
                <div style="max-height:260px; overflow-y:auto;">
                    <table style="width:100%; border-collapse:collapse; text-align:left;">
                        <thead>
                            <tr style="color:var(--tx2); font-size:0.8rem;">
                                <th style="padding:8px; border-bottom:1px solid var(--bd);">${L==='ar'?'التاريخ':'Date'}</th>
                                <th style="padding:8px; border-bottom:1px solid var(--bd);">${L==='ar'?'الصنف':'Item'}</th>
                                <th style="padding:8px; border-bottom:1px solid var(--bd);">${L==='ar'?'القيمة':'Value'}</th>
                            </tr>
                        </thead>
